...
void output(unsigned char *buf,int wrap, int xsize,int ysize, FILE* pf)
{
    int i;
    for(i=0;i<ysize;i++)
    {
        fwrite(buf + i * wrap, 1, xsize, pf);
    }
}
    ...
    for(;;) {
        ...
        while (size > 0) {
            ...
            if (got_picture) {
                ...
                output(picture->data[0], picture->linesize[0],c->width, c->height, fout);
                output(picture->data[1], picture->linesize[1],c->width/2, c->height/2, fout);
                output(picture->data[2], picture->linesize[2],c->width/2, c->height/2, fout);
                /*
