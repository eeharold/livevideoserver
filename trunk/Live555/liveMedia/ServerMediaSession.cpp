...
//根据媒体名称到hash表中寻找相应的ServerMediaSession对象
...
::lookupByName(UsageEnvironment& env, char const* mediumName,
  ...
  {
    DEBUG_LOG(ERR, "Can't find ServerMediaSession: %s", mediumName);
    return False;
  }

  if (!medium->isServerMediaSession()) {
    DEBUG_LOG(ERR, "%s is not a 'ServerMediaSession' object", mediumName);
    env.setResultMsg(mediumName, " is not a 'ServerMediaSession' object");
    return False;
  }

  DEBUG_LOG(INF, "Found ServerMediaSession: %s", mediumName);
...
ServerMediaSession::ServerMediaSession(UsageEnvironment& env,
  ...
  DEBUG_LOG(INF, "Construct ServerMediaSession, streamName = %s, "
    "fInfoSDPString = %s, fDescriptionSDPString = %s, fMiscSDPLines = %s", 
    fStreamName, fInfoSDPString, fDescriptionSDPString, fMiscSDPLines);
  gettimeofday(&fCreationTime, NULL);
}

ServerMediaSession::~ServerMediaSession() {
  DEBUG_LOG(INF, "Deconstruct ServerMediaSession");
