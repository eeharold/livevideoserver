...
RTCPInstance::RTCPInstance(UsageEnvironment& env, Groupsock* RTCPgs,
  ...
  DEBUG_LOG(INF, "Construct RTCPInstance: allocal OutPacketBuffer"
    "(preferredPacketSize = %u, maxPacketSize = %u)", preferredPacketSize, maxPacketSize);
...
void RTCPInstance::incomingReportHandler(RTCPInstance* instance,
                     int /*mask*/) {
  DEBUG_LOG(INF, "RTCPInstance: Handle incoming request begin");
  instance->incomingReportHandler1();
  DEBUG_LOG(INF, "RTCPInstance: Handle incoming request end");
}
