...
MPEG4VideoStreamFramer::MPEG4VideoStreamFramer(UsageEnvironment& env,
  ...
  DEBUG_LOG(INF, "Construct MPEG4VideoStreamParser, flag = %d", createParser);
...
unsigned MPEG4VideoStreamParser::parse() {
  DEBUG_LOG(INF, "MPEG4VideoStreamParser, fCurrentParseState = %d", fCurrentParseState);
  try {
    switch (fCurrentParseState) {
    case PARSING_VISUAL_OBJECT_SEQUENCE: { // 0 可视化对象序列
      return parseVisualObjectSequence();
    }
    case PARSING_VISUAL_OBJECT_SEQUENCE_SEEN_CODE: { // 1 可视化对象序列 开始
      return parseVisualObjectSequence(True);
    }
    case PARSING_VISUAL_OBJECT: {  // 2 可视化对象
      return parseVisualObject();
    }
    case PARSING_VIDEO_OBJECT_LAYER: { // 3 视频对象层
      return parseVideoObjectLayer();
    }
    case PARSING_GROUP_OF_VIDEO_OBJECT_PLANE: { // 4 视频对象平面组
      return parseGroupOfVideoObjectPlane();
    }
    case PARSING_VIDEO_OBJECT_PLANE: { // 5 视频对象平面
      return parseVideoObjectPlane();
    }
    case PARSING_VISUAL_OBJECT_SEQUENCE_END_CODE: { // 6 可视化对象序列 结束
...
#endif
  DEBUG_LOG(INF, "MPEG4VideoStreamParser::parse end, the parsing got interrupted");
...
::parseVisualObjectSequence(Boolean haveSeenStartCode) {
  DEBUG_LOG(INF, "parsing VisualObjectSequence");

  usingSource()->startNewConfig();
  u_int32_t first4Bytes;
  
  if (!haveSeenStartCode) //是否已经找到 visual_object_sequence_start_code
  {
      //寻找start_code(4字节)， test4Bytes里机关重重。
      while ((first4Bytes = test4Bytes()) != VISUAL_OBJECT_SEQUENCE_START_CODE) 
      { 
          DEBUG_LOG(WAN, "Ignoring non 'visual_object_sequence_start_code': 0x%08x", first4Bytes);
          get1Byte(); //后移一个字节
  ...
  //最开始的4个字节是visual_object_sequence_start_code
  save4Bytes(first4Bytes);
  DEBUG_LOG(INF, "visual_object_sequence_start_code = %08x", first4Bytes);

  // 下一个字节是 "profile_and_level_indication":
  u_int8_t pali = get1Byte();
  saveByte(pali);
  usingSource()->fProfileAndLevelIndication = pali;
  DEBUG_LOG(INF, "profile_and_level_indication = %02x", pali);

  // Now, copy all bytes that we see, up until we reach
  // a VISUAL_OBJECT_START_CODE:
  // 这里都是用户数据，直接拷贝。直到visual_object_start_code
  ...
  DEBUG_LOG(INF, "%d Bytes: user data...", i*4);

  setParseState(PARSING_VISUAL_OBJECT);//跳到第二个状态
  ...
  DEBUG_LOG(INF, "curFrameSize = %d", curFrameSize());
...
unsigned MPEG4VideoStreamParser::parseVisualObject() {
  DEBUG_LOG(INF, "parsing VisualObject");

  // Note that we've already read the VISUAL_OBJECT_START_CODE
  // 已经在上一个状态中读取了VISUAL_OBJECT_START_CODE，所以这里无需再读
  save4Bytes(VISUAL_OBJECT_START_CODE);
  DEBUG_LOG(INF, "visual_object_start_code = %08x", VISUAL_OBJECT_START_CODE);
  ...
  DEBUG_LOG(INF, "is_visual_object_identifier = %d", is_visual_object_identifier);
  
  u_int8_t visual_object_type;
  if (is_visual_object_identifier) 
  {
      DEBUG_LOG(INF, "visual_object_verid = %x", (nextByte&0x78)>>3);
      DEBUG_LOG(INF, "visual_object_priority = %x", (nextByte&0x07));
  ...
  DEBUG_LOG(INF, "visual_object_type = %x", visual_object_type);
  ...
  DEBUG_LOG(INF, "video_object_start_code = %08x", next4Bytes);

  setParseState(PARSING_VIDEO_OBJECT_LAYER);//跳到第三个状态
...
unsigned MPEG4VideoStreamParser::parseVideoObjectLayer() {
  DEBUG_LOG(INF, "parsing VideoObjectLayer");
  ...
  DEBUG_LOG(INF, "video_object_layer_start_code = %08x", next4Bytes);
  ...
  DEBUG_LOG(INF, "user data = %d bytes", (next4Bytes-1)*4);

  analyzeVOLHeader();

  setParseState((next4Bytes == GROUP_VOP_START_CODE)
                ? PARSING_GROUP_OF_VIDEO_OBJECT_PLANE
		: PARSING_VIDEO_OBJECT_PLANE);//跳到第四或者第五个状态
...
unsigned MPEG4VideoStreamParser::parseGroupOfVideoObjectPlane() {
  DEBUG_LOG(INF, "parsing GroupOfVideoObjectPlane");

  // Note that we've already read the GROUP_VOP_START_CODE
  save4Bytes(GROUP_VOP_START_CODE);
  DEBUG_LOG(INF, "group_vop_start_code = %08x", GROUP_VOP_START_CODE);
...
unsigned MPEG4VideoStreamParser::parseVideoObjectPlane() {
  ...
  DEBUG_LOG(INF, "vop_coding_type: %d(%c), modulo_time_base: %d, vop_time_increment: %d",
  ...
  saveToNextCode(next4Bytes);
  //前面的头有9个字节,所以这里减9
  DEBUG_LOG(INF, "Until see 0x000001xx, we have copied %d bytes", curFrameSize()-9);
  ...
  default: {
   DEBUG_LOG(INF, "parsing VideoObjectPlane, default case...");
...
unsigned MPEG4VideoStreamParser::parseVisualObjectSequenceEndCode() {
  DEBUG_LOG(INF, "parsing visual_object_sequence_end_code");
