...
::OnDemandServerMediaSubsession(UsageEnvironment& env,
  ...
  DEBUG_LOG(INF, "Construct OnDemandServerMediaSubsession, hostname = %s", fCNAME);
}

OnDemandServerMediaSubsession::~OnDemandServerMediaSubsession() {
  DEBUG_LOG(INF, "Deconstruct OnDemandServerMediaSubsession");
  delete[] fSDPLines;
...
//ServerMediaSession::generateSDPDescription()中被调用(处理DESCRIBE命令时使用)
//生成sdpline
char const* OnDemandServerMediaSubsession::sdpLines() {
  if (fSDPLines == NULL) {
    ...
    DEBUG_LOG(INF, "Estimate bit rate = %d, rtpPayloadType = %d", estBitrate, rtpPayloadType);
...
::startPlaying(Destinations* dests,
  ...
  DEBUG_LOG(INF, "StreamState::startPlaying");
...
void StreamState::endPlaying(Destinations* dests) {
  DEBUG_LOG(INF, "StreamState::endPlaying");
  if (dests->isTCP) {
    if (fRTPSink != NULL) {
...
void StreamState::reclaim() {
  DEBUG_LOG(INF, "StreamState::reclaim");
