...
Boolean MediaSink::startPlaying(MediaSource& source,
  ...
  DEBUG_LOG(INF, "MediaSink::startPlaying");
...
void MediaSink::stopPlaying() {
  DEBUG_LOG(INF, "MediaSink::stopPlaying");
...
OutPacketBuffer::OutPacketBuffer(unsigned preferredPacketSize,
  ...
  DEBUG_LOG(INF, "Construct OutPacketBuffer: head = %p, size = %u", fBuf, fLimit);

  DEBUG_LOG(INF, "OutPacketBuffer: "
    "preferredPacketSize=%u, maxPacketSize = %u; "
    "maxSize = %u, size = %u, maxNumPackets = %u", 
    fPreferred, fMax, maxSize, fLimit, maxNumPackets);
...
OutPacketBuffer::~OutPacketBuffer() {
  DEBUG_LOG(INF, "Deconstruct OutPacketBuffer: head = %p", fBuf);
  delete[] fBuf;
}

//将字符串from复制numBytes个字节到内部缓冲区
