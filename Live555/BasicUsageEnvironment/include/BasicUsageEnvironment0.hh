...
private:
  ...
  unsigned fCurBufferSize; //缓冲区当前大小
  unsigned fBufferMaxSize; //缓冲区最大大小，默认是1000
};
...
protected:
  ...
  DelayQueue fDelayQueue; //保存延迟任务的队列

  // To implement background reads:
  HandlerSet* fReadHandlers;//读事件响应函数链表
  int fLastHandledSocketNum;//上一次被处理的socket句柄
};
