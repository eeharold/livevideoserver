...
::computePresentationTime(unsigned numAdditionalPictures) {
  DEBUG_LOG(INF, "computePresentationTime, numAdditionalPictures = %u", numAdditionalPictures);
...
//#ifdef DEBUG
  if(isWriteLog())
  {
    static struct timeval firstPT;//第一次调用这个函数时的时间
    if (firstPT.tv_sec == 0 && firstPT.tv_usec == 0) 
        firstPT = fPresentationTime;
    
    struct timeval diffPT; //与第一次调用这个函数时的时间差
    ...
    DEBUG_LOG(INF, "computePresentationTime(%d) -> %s.%06ld [%lu.%06ld]",
      numAdditionalPictures, 
      timeToStr(fPresentationTime.tv_sec), 
      fPresentationTime.tv_usec, 
      diffPT.tv_sec, diffPT.tv_usec);
  }
...
void MPEGVideoStreamFramer::doGetNextFrame() {
  DEBUG_LOG(INF, "MPEGVideoStreamFramer::doGetNextFrame");
...
void MPEGVideoStreamFramer::continueReadProcessing() {
  DEBUG_LOG(INF, "MPEGVideoStreamFramer::continueReadProcessing");
  
  unsigned acquiredFrameSize = fParser->parse();//根据协议内容解析文件,并拷贝到缓冲区
  DEBUG_LOG(INF, "acquiredFrameSize = %u", acquiredFrameSize);
  ...
  if (acquiredFrameSize > 0) {
    ...
    DEBUG_LOG(INF, "fDurationInMicroseconds = %d((%d*1000000)/%f)", 
