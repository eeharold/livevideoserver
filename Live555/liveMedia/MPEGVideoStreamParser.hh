...
protected:
  ...
  void save4Bytes(u_int32_t word) {
    ...
    //将word转换字节序后保存到发送缓冲区中。
    //attention: 为什么不用htonl?????
...
protected:
  ...
  unsigned char* fStartOfFrame;//缓冲区起点
  unsigned char* fTo;//缓冲区当前位置
  unsigned char* fLimit;//缓冲区的结束点
  unsigned fNumTruncatedBytes;
  unsigned curFrameSize() { return fTo - fStartOfFrame; }
  unsigned char* fSavedTo;//fSavedTo之前的缓冲区是已经保存过的了
